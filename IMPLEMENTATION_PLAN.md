...
**S1c 会话治理与分组**
- Outcome：omp-supervisor 完整治理（每活跃会话一个、空闲回收、数量上限——池参数在此实测定参）；会话分组侧栏、三场景、fork、中断。另按 2026-09-24 S1e grill 裁定归入本阶段：**重新生成**（与中断/继续同批，复用回合语义）、**审批条**（允许/拒绝/超时自动通过——需 omp 退出 yolo 的权限模型，与 F-OPS-1 同批定）、**对话内搜索**（纯前端，落在对齐后的消息呈现层上）。
