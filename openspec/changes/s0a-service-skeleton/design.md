...
- Regression rows:
  ...
  - Malformed ledger, non-prefix/unknown receipt, same-name incompatible guard object, case-variant/receipt-prefix-inconsistent extra ledger trigger, or `sqlite_sequence` mismatch -> `openDb` fails before any new migration effect or receipt; unrelated-table triggers remain allowed.
