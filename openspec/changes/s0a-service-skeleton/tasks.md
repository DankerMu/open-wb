...
- [x] 1.3 启动入口与命令面：`server.ts`（唯一 listen/DB owner、启动日志输出模块清单）、production-only TS build + migration asset copy、`make dev` 与 `npm run start --workspace server`、knip entry、`.gitignore var/`
  - Issue #7 fixture: high（upstream compact因production entrypoint/CLI、env config、DB file output、listen/close shared-state lifecycle与release asset命中mandatory expanded/high trigger）；repair intensity: high；minimal slice: unique entry + build/start/dev/knip/gitignore原子接线。
  - Change boundary: `server/src/server.ts`及其pure config tests、server build config/package scripts、Makefile dev、knip、`.gitignore`与本fixture；不改`createApp`、DB/migration/auth/http/web产品合同，不实现#16 hurl或CI wiring。
  - Required evidence:
    - TDD red-proof：pre-change `npm run build/start --workspace server`与`make dev`因command/entry缺失而红，`git check-ignore var/dev.db`失败；不是新测试syntax/import红，不留stash/probe；
    - exact config：own `HOST/PORT/DB_PATH/STATIC_ROOT` missing→`127.0.0.1/3000/<repo>/var/dev.db/<repo>/web/dist`；repo root由entry `import.meta.url` identity推导，repo-root commands与foreign-cwd absolute compiled entry得到同一relative-path结果；absolute path不改写，exact`:memory:`不mkdir；unknown env不消费；
    - strict negative config：PORT empty/whitespace/sign/fraction/exponent/0/65536，HOST empty/whitespace-only，DB_PATH/STATIC_ROOT explicit empty均在mkdir/open/listen前nonzero，application stderr exact一行`{"event":"server_start_failed"}`、stdout无success、无extra/raw config；HOST其它nonempty value与path value不trim/coerce/clamp/fallback并由listen/owner处理；import `server.ts`不命中ESM main guard时silent且不得mkdir/open/listen/register signal；
    - build/packaging：clean `npm run build --workspace server`只emit `src` production JS到ignored `server/dist`，不emit tests/coverage；compiled imports可由Node 24离线解析；dist `migrations/`递归tree（含`.sql`、ignored sentinel与nested fixture）相对path inventory/bytes exact等于tracked source且无missing/extra，避免compiled `import.meta.url`丢迁移；连续build删除stale output；package/lock无新dependency；
    - command identity：`make dev`只exec同一workspace `start` owner；start在clean state build后执行compiled `dist/server.js`；knip把`src/server.ts`视为entry，`.gitignore`排除`var/`与dist runtime output；
    - real process success：repo root两条public command共享同一owner；build后从foreign cwd以absolute compiled-entry且四项unset证明相同defaults/repo-root DB，再以temporary absolute DB/static、free port/loopback override启动，health exact200且info/auth/static/guard合同不变；application stdout恰一行LF JSON、no extra key，exact `{event,host,port,modules:[core/db,auth,http]}`且只在listen后出现，application stderr无record；任何stream不含unknown-env sentinel/cookie/password/session；custom DB完成全部migration且default var不误建；
    - DB parent/file policy：non-`:memory:`只recursive mkdir `dirname(DB_PATH)`；`:memory:`与STATIC_ROOT零mkdir；nested missing parent成功，existing file/non-directory或不可创建/打开parent在listen前nonzero，default/static无误建；
    - failure/cleanup：occupied port、HOST listen rejection、DB open/migration与post-listen stdout sink failure均nonzero、无success；stderr可写时application record仅exact generic `{"event":"server_start_failed"}`，不得有EPIPE/raw stack（允许独立SQLite ExperimentalWarning）；stdout+stderr均不可写时record可物理缺席但仍nonzero/无unhandled stack；所有lane关闭app/DB且successor立即复用exact port/file；
    - signal/race：per-entry AbortSignal传给Fastify listen；SIGINT/SIGTERM在listen invoke/pre-bind窗口取消后到bind、无startup record、正常退出，去掉signal的mutant须留下可接受连接的listener而变红；成功后SIGINT/SIGTERM与重复/混合signal共享幂等shutdown，停止listener后关DB；重复启动同DB成功且migration receipt不增；每个probe finally杀进程、关handle并删temp；
    - compatibility：existing createApp caller-owned DB、#5/#6/#8/#9/#10/#19、web/kbservice不变；focused config tests、server full、`make check`、guardrails、strict OpenSpec、knip/jscpd/size/naming/secret/debug/skip/stash scans全绿。
  - Non-goals: durable listener unit test（真实常驻startup regression由#16 hurl接管；本issue只保pure config unit + Phase2 controlled real-process evidence）、watch/hot reload、dotenv、TLS、daemonization、PID file、systemd/docker/CI、schema/env secrets、multi-process DB coordination。
...
- [x] 4.1 hurl 用例集（healthz/info/登录三态含信封文案断言/守卫 401/伪造 session 401/登出/深链 fallback/API 404 信封）+ `make smoke`（缺 hurl 显式失败并打印指引）
  ...
  - Required evidence:
    ...
    - command contract：`SMOKE_BASE_URL` omitted→`http://127.0.0.1:3000`，override origin（无 trailing slash）只传成一个 Hurl `base_url=<完整值>` argv；含 whitespace、quote、semicolon、`$()`、backtick、`#`、`&`、newline 或 `$(shell ...)` 形状的无效值与hostile PATH component保持 inert bytes、零side effect，任何discovered path不回写recipe source，fake Hurl nonzero仍使Make nonzero；`make smoke` 以 `--test --jobs 1 --retry 0`执行全部 top-level `smoke/*.hurl`并原样传播失败；不 build/start/stop/install 服务或工具，不写 cookie jar/report。
    - child environment：preflight与真实Hurl均只继承caller PATH并以literal `hurl`解析，真实Hurl direct child environment恰只有PATH；ambient `HURL_NO_ASSERT/HURL_RETRY/HURL_USER/HURL_VARIABLE_*`、future Hurl option env、HOME/XDG config与proxy/credential vars均不进入child。Wrong-status + no-assert仍nonzero；infinite-retry + closed port在bounded timeout内prompt nonzero；ambient user不产生Authorization header；exported shell function/alias或ambient `HURL=/path`不能冒充/改指binary。
