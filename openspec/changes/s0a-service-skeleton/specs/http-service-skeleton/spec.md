...
### Requirement: 统一错误信封
所有本阶段可预期 `/api/*` 应用错误与 API 404 SHALL 使用统一信封 `{ "error": { "code": "<snake_case>", "message": "<可直接展示的中文文案>" } }`；typed definition map 固定为 `invalid_credentials`(401, `账号或密码不正确`)、`account_disabled`(403, `该账号已停用，请联系管理员`)、`unauthorized`(401, `请先登录`)、`not_found`(404, `请求的资源不存在`)；处理器 SHALL 落在 `http/` 横切层。意外 programmer error 不得被误标成这四种语义错误，仍为 5xx。

#### Scenario: 四码信封形状一致
- WHEN 测试路由分别抛出四种 typed application error
...
- THEN 返回 5xx，且 body 不得声称四个 typed semantic code 中任一个
