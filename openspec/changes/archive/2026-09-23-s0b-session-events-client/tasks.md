...
- [x] 2.4 Read-only independent source reviews and exact-head required CI pass with bounded fix gate. PR245 head `7903eea531723512fdf6500ef2347cba1da4288f`, round2 clean after one coverage-only fix pass; CI35880393231 all eight jobs successful.

## 3. Delivery
- [x] 3.1 Synchronize parent plan, merge source PR/close issue, then prepare canonical archive with recorded evidence. PR245 merged; independent archive PR/CI remains the external gate below.

Archive delivery remains an external parent-workflow gate: independently validate and merge its PR/CI before #104.

Local evidence: `/tmp/open-wb-issue93-evidence/`. Twenty-eight new tests and 434 total web tests pass; historical connector rejects five regressions. Eight disposable cursor/recovery/decoder mutants are rejected; one initially survived incomplete stale-load assertions, then was rejected after adding no-error/no-close assertions and restoring GREEN. Four earlier public counterexamples and the captured-then getter counterexample retain semantic RED logs. Chromium native and foreign-realm lanes both pass restoration and one dedicated no-change stability trial (zero browser errors); screenshots are protocol harnesses, not the future #104 product page. Missing-module/function tracers are setup RED only. Source review and exact-head CI passed; independent archive delivery remains pending.
