...
## What Changes
- Lossless re-drain in `createApp`: once the app is closing (flag set by the first root `preClose`, registered right after `fastify()` so no failing or timed-out module hook can skip it), every completed response triggers `app.server.closeIdleConnections()` on the next macrotask. Node only reclaims connections without an unfinished request, so no in-flight request is cut.
- Named listener drain budget `LISTENER_CLOSE_BUDGET_MS = 2_000`, armed by a delegating wrapper around the app's own `server.close()`, which Fastify calls once after the whole preClose chain whatever its outcome (i.e. after native runtime shutdown, so it never truncates the native TERM/KILL budget). Only if connections are still open when it expires does it escalate with `closeAllConnections()` and invoke an optional `onListenerForceClose` callback; the timer is cleared/unref'd so a normal close is never delayed by it. Default rationale: native worst case 8 s + listener 2 s = 10 s, the docker default stop grace.
