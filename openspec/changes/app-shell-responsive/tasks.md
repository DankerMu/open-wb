...
- [ ] 2.1 新增 `web/test/media-query-support.ts`（从 `theme-provider.test.tsx` 抽出 `createMediaQuery` + `installMatchMedia(resolve)`，theme 测试改 import）；新增 `web/test/app-shell-responsive.test.tsx`（R1–R10 含 R2c/R4b/R9b）；`web/test/sidebar.test.tsx` 删 `narrowCss` 与依赖横条 CSS 的两处断言。先红。
- [ ] 2.2 `web/src/lib/viewport.ts`（`SHELL_NARROW_QUERY`/`useMediaQuery`，matchMedia 缺失恒 false）。
- [ ] 2.3 `web/src/features/auth/provider.tsx` `logoutPending` 上提 + `footer.tsx` 改读 Provider（`settings-footer.test.tsx` 零改动仍绿）。
- [ ] 2.3b `web/src/routes/shell/sidebar.tsx` 覆盖层变体（`variant: "overlay"` + `onNavigate`，无品牌区/折叠按钮，`data-variant`）；`sidebar.css` 删 `≤760` 横条块、加 `.sidebar[data-variant="overlay"]` 规则。
- [ ] 2.4 `web/src/routes/shell/topbar.tsx` `onOpenNav` + `打开导航` 按钮（header 首子节点，欢迎态窄条）；`topbar.css` 按需对齐规则。
- [ ] 2.5 `web/src/routes/shell/app-shell.tsx`：`useMediaQuery` 分支、`Drawer side="left" width=288 title="导航"`、`navOpen` 瞬时状态与宽屏复位；`styles.css` `≤760` 块删 `flex-direction: column`。2.1 转绿，`web/test` 全绿。
- [ ] 2.6 反向注入九项各红并回退（design Required evidence）。

## 3. Verification
- [ ] 3.1 `make check` exit 0（size-guard、knip、jscpd、guardrails）。
- [ ] 3.2 `npm run build --workspace web` exit 0。
- [ ] 3.3 `ci-compiled-server.sh ui-walk`（CI 环境变量，单 project）exit 0；`git diff --stat web/src/ui web/e2e web/playwright.config.ts` 为空。
