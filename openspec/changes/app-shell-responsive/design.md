...
- (R8) 预置偏好：`localStorage.setItem("workbuddy-sidebar", "collapsed")` 后窄屏挂载并打开 → 覆盖层标签可见（展开态），`getItem` 仍为 `collapsed`、`setItem` spy 未调用；`emit(false)` → 文档流 `aside[data-collapsed="true"]`。
