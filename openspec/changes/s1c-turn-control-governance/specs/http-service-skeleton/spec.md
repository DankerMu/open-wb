# Spec delta: http-service-skeleton（S1c A 修改）
...
- **WHEN** 有效会话携相同错误访问八条归属POST身份之外的其他已注册protected API route，或programmer error仅携带`statusCode=400/413`/伪造allowlist code/validation-shaped fields
...
- **WHEN** genuine allowlisted Fastify content-parser error is mapped for the production-mounted POST matched /api/sessions/:id/prompt, /v1/chat/completions, /api/workspaces, /api/workspaces/:id/dirs, /api/sessions/:id/fork or /api/sessions/:id/approvals/:approvalId
- **THEN** the mapper returns exact400 bad_request without rawdetails; nonPOST, lookalike/raw-concrete identities, the bodyless stop/regenerate routes and registered unowned routes staygeneric500; ownership is decided solely by the shared mapper's exact eight-identity set, not by whichever module registered the route
...
- **THEN** exact400 bad_request is returned before their handlers run, with the route's own no-store preserved; the eight-owner policy covers these two identities alongside the six other owners
...
- **WHEN** the same inputs hit the bodyless POST /api/sessions/:id/stop or /api/sessions/:id/regenerate, or arrive unauthenticated on any of the four routes
- **THEN** stop/regenerate are not parser owners and keep the generic boundary for genuine parser errors, and unauthenticated requests receive401 before parser or handler work
