...
Must preserve: official OMP_BIN supplied by config (#86), exact parent spec ordering and optional-field semantics; no spread of process.env, no persistent global environment mutation.
...
Seams under test: injected spawn receives exact values with real temporary filesystem; a real Node child probe verifies the effective env/cwd and argument boundaries under polluted parent environment. The probe is a tiny diagnostic payload, not another fake RPC implementation.
