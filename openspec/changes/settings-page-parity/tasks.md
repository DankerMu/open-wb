...
- [ ] 2.1 抽 `web/test/settings-support.tsx`，把 `describe("settings route")` 从 `settings-footer.test.tsx` 迁到新 `web/test/settings-page.test.tsx`；在现实现上保持全绿，记录迁移前后用例数。
- [ ] 2.2 `settings-page.test.tsx` 新增 S1–S7，并按改写规则 2–3 更新迁入用例锚点。先红。
- [ ] 2.3 `page.tsx` 结构、`settings.css`、`styles.css` 删除旧规则并 import、`ui.css` 增 `.ui-sr-only`。2.2 转绿，`web/test` 全绿。
- [ ] 2.4 反向注入十二项各红并回退（design Required evidence）。

## 3. Verification
- [ ] 3.1 `make check` exit 0（size-guard、jscpd、knip、ui-guardrails）。
