...
- THEN 直接 spawn 时子进程收到的 env 键集合精确等于白名单，四者均不存在；sudo 形态时 sudo 进程 env 键集合精确等于白名单、argv 不含任何 `KEY=value`，omp 子进程键集 ⊇ 白名单、四个预置键均不存在、`HOME`/`PI_CODING_AGENT_DIR` 值为传入值；`WORKBUDDY_MODEL_TOKEN` 为该会话 64 位 lowercase hex
...
- THEN 可执行为 `sudo`；argv 精确为 `-n -u omp --preserve-env=PATH,LANG,TMPDIR,HOME,PI_CODING_AGENT_DIR,WORKBUDDY_MODEL_TOKEN -- <OMP_BIN> --mode rpc …`（完整 omp 参数，不含任何 `KEY=value`）；sudo 进程 env 为 `PATH`、（父进程有则）`LANG`、`TMPDIR`、`HOME=<OMP_STATE_DIR>/home`、`PI_CODING_AGENT_DIR=<OMP_STATE_DIR>/agent`、`WORKBUDDY_MODEL_TOKEN=<64hex>`
