...
Archive coordination: #120 配置/sudo 与 unsafe-PATH 两项、#126 私有状态要求已晋升至 `omp-uid-isolation`；父归档保留完整ompUser链路/sudo-only安全PATH、before-open600、nonregular失败不改模式、新建2770而既有目录不改、不改umask/DB父0700，不用旧组合文本覆盖。真实uid/proc/PAM仍为#131/#132，部署负责组归属，stat/chmod TOCTOU不在可信路径合同内。
