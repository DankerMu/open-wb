...
- [ ] 5.3 `server/test/support/fake-omp.mjs` 增 `probe` 模式（prompt `probe:<pid>:<writePath>` → 先以自身 uid 写 `<writePath>`，再回 `uid=… gid=… env=<sorted keys> home=<$HOME> agent=<$PI_CODING_AGENT_DIR> environ=<EACCES|readable|errno> wrote=<ok|errno>` 的 `text_delta` + 正常 `agent_end`）+ 契约单测（依赖 S0b #87）
