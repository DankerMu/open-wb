## Why
- #191 — `server/test/omp-process.test.ts` `readChildStdout` (:600-608) attaches `stdout` `'data'` and returns after `waitExit` (:666-678). `waitExit` resolves immediately when `child.exitCode !== null` (:667) and otherwise on `'exit'`. Node does not guarantee stdio is drained at `'exit'`; only `'close'` follows stream closure. The `exitCode` shortcut is unreachable on this path (`spawnOmp` spawns synchronously after a microtask, so listeners attach before any I/O phase); the observed `envDump = {}` fits the remaining race: with listeners attached, the parent can dispatch `'exit'` before the pipe `'data'`. The default-spawn case (:242-268) has the same shape. There is no timeout, so a stream that never closes would only hit the generic vitest timeout without diagnostics.
