...
- THEN 折叠后侧栏宽 48px、导航只显示图标、列表区不渲染且 hover/聚焦出现 Tooltip 标签，`localStorage.workbuddy-sidebar=collapsed`，reload 保持折叠；菜单只含 `退出登录` 一项，Escape 关闭并恢复触发器焦点
...
#### Scenario: 四路由可达
- WHEN 已登录用户依次访问四个路由
- THEN `/` 显示会话页与输入框、侧栏列表区含会话列表与 `新建会话`，`/files` 显示工作空间页、空间切换器与目录树区，`/center` 保持占位壳，`/settings` 保持设置页；侧栏高亮当前 tab，无浏览器控制台报错
