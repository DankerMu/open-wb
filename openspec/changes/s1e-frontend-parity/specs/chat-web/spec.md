# Spec delta: chat-web（S1e 修改）

> 按仓内先例整段重述 `会话页` Requirement（含全部 Scenario），归档时整段替换。`API 客户端扩展`、`纯会话视图归约`、`事件流消费与续流` 不变。demo 行号指 `resource/workbuddy-live-demo.html`。

## MODIFIED Requirements

### Requirement: 会话页
`/` SHALL render the account-owned session list, new-session action, message area and composer. The page-level level-1 heading follows spa-shell: in welcome state it is the hero `WorkBuddy，我帮你`; with a selected session it is the topbar breadcrumb container (accessible name `我的工作 / <title>`); the page SHALL NOT render its own page-level `<h1>` (headings inside rendered Markdown are content, not page headings). List SHALL retain server updatedAt-descending order and show server title or `新会话` plus a status element (`role="status"`, accessible name `<title> 运行中|已完成|失败`, visible dot with `running` pulsing via `ui-pulse`, visually-hidden text `运行中|已完成|失败`). Selection/new SHALL update `?session=<id>` while preserving unrelated search/hash; refresh and Back SHALL restore the selected session. Missing selection or inaccessible target SHALL show the **welcome state** and composer, never auto-select first session. An inaccessible initial GET404 SHALL replace-remove the session parameter and SHALL NOT open EventSource; other errors SHALL remain visible without displaying another session's history.

**Welcome state** (demo:2537-2567) SHALL show hero `WorkBuddy，我帮你`, one row of quick chips (static list ported from demo `QUICK_PROMPTS` for the default 日常办公 scene only; scene pills themselves belong to S1c and are not rendered), the composer card, a `不知道做什么，试试最佳实践案例` section with five static playbook cards (ported from demo `PLAYBOOKS`, demo:2553-2561) and a `换一批` action that rotates within the static set (`查看更多` is not rendered because its target `/center` is undelivered), and the disclaimer `内容由 AI 生成，请核实重要信息`. Clicking a chip or card SHALL only fill the composer draft, never send.
...
**Messages** (demo:2378-2406) SHALL render user messages as right-aligned bubbles that preserve complete text and whitespace (`white-space: pre-wrap`, safe rendering) and assistant messages as left-aligned plain blocks with an assistant avatar mark (`BrandMark`); assistant text SHALL be rendered through the shared safe Markdown renderer (`web/src/lib/md-render.ts`, relocated from the files feature with its provenance header intact), SHALL preserve complete text (whitespace per Markdown semantics; `复制` copies the raw text) and a running assistant SHALL show a blinking caret (`ui-caret`) after the last character. Each assistant message SHALL have an action row with a single `复制` button (copies raw text via `navigator.clipboard.writeText`; when the clipboard API is unavailable or rejects, a `Toast` `复制失败` is shown and no exception escapes; regenerate/like/dislike belong to S1c or are dropped). **Step cards** (demo:2218-2242) SHALL show a header (`Icon terminal` for `bash`, `Icon wrench` otherwise, step name, and a status badge with `role="status"`, visible text `运行中|已完成|失败` mapped from running/done/failed and accessible name `<step name> 运行中|已完成|失败`) and a one-line summary derived from detail: for JSON detail the first non-empty `text`/`content` string or the first key/value; for non-JSON detail the first non-empty line; empty detail yields an empty summary; all truncated to 120 code points; full raw detail SHALL stay available behind a collapsed `<details>` `原始输出` (collapsed by default, not open). No fabricated time or todo state. A `回到最新` floating button SHALL appear when the transcript is scrolled more than one viewport above the bottom and disappear after clicking or reaching the bottom; new events SHALL auto-scroll only when already at the bottom.
...
#### Scenario: 消息与步骤呈现
- WHEN 一次已完成回合的快照包含用户消息、含 Markdown（标题 + 代码块）的助手正文与一条 detail 为 JSON 的 bash 步骤
- THEN 用户消息右侧气泡且多行文本换行保留、助手正文渲染出 heading 与 code 元素（源 HTML 转义）、步骤卡头显示 `bash` 与 `已完成`（`role=status` 名 `bash 已完成`）、摘要行 ≤120 码点且 `原始输出` 折叠未展开；`复制` 后剪贴板等于助手原始文本，clipboard 缺失或 reject 时出现 Toast `复制失败` 且无未捕获异常；running 状态下正文末尾存在 `ui-caret`，会话列表当前项 status 文本为 `运行中`

#### Scenario: 回到最新
- WHEN 长历史下用户上滚超过一屏，随后新 delta 到达；再点击 `回到最新`
- THEN 上滚期间不自动滚动且按钮可见；点击后滚到底部、按钮消失，之后新 delta 保持自动跟随
...
- THEN exactly one session and prompt are created, URL selects that ID, topbar breadcrumb shows the server title, user and assistant appear in server order without duplicates, text grows, step detail/status changes, server title appears and composer unlocks at done
...
- THEN the page shows the welcome state and composer, no first-session fallback and no source for the inaccessible target; invalid query removal preserves other search/hash
