...
- THEN exactly one session and prompt are created, URL selects that ID, topbar breadcrumb shows the server title, user and assistant appear in server order without duplicates, text grows, step status/output changes, server title appears and composer unlocks at done
