...
`make ui-shots` SHALL 只消费由 caller 启动、可从 `UI_SHOTS_BASE_URL`（缺省 `http://127.0.0.1:3000`）访问的真实服务与仓库内 `resource/workbuddy-live-demo.html`（`file://`），不得 build/start/stop/安装浏览器；实现为 `npm run ui-shots --workspace web` → `web/e2e/ui-shots.mjs`（Playwright chromium，与 ui-walk 同一依赖）。目标 SHALL 对六个格（视口 `1440×900`、`1024×768`、`390×844` × `light`/`dark`）截取五个态，态名固定为 `login-default`（登录前）、`chat-welcome`（登录后 `/` 无会话）、`chat-done`（一次已完成回合的会话）、`files-readme`（已选 `smoke-fixture` 且预览 `readme.md`）、`settings-default`：app 侧主题经 `addInitScript` 预置 `localStorage.workbuddy-theme`，以 `zhangsan`/`demo` 登录；`chat-done` 由脚本在首格新建会话、发送固定提示 `请用一句话介绍你自己` 并等待会话 status 为 `已完成`（上限 60s）后复用该会话 id 于其余各格；demo 侧（`resource/workbuddy-live-demo.html`，`file://`）以 `addInitScript` 预置其主题存储键、以 `[data-quick="zhangsan"]` 快捷登录，`files-readme` 取 demo 首个工作空间的首个 `.md`，`chat-done` 取 demo 预置会话中首个已完成会话（所选 id 与主题键为脚本常量并注明 demo 行号）。输出到 `UI_SHOTS_OUT`（未设置时由脚本取 `var/ui-shots/<UTC 时间戳>/`）：每张 PNG 命名 `<app|demo>-<state>-<width>-<theme>.png`，并生成 `index.html` 对照表（每行 demo 左、app 右，标注格与态）。app 侧 SHALL 走 `?ws=` 与真实 REST，不注入假数据；每张 app 截图前 SHALL 断言 `document.documentElement.scrollWidth <= window.innerWidth`，且 DOM 文本/`title`/aria 属性不含任何 workspace `root` 绝对路径值。任一页面导航失败、断言失败或 console error SHALL 使目标非零并保留已产出文件。

#### Scenario: 六格产物齐全
- WHEN caller 以 fresh DB、真实 `web/dist`、夹具已复制到 `<SANDBOX_ROOT>/u1/`、`OMP_BIN` 指向已校验 omp 且模型上游可用（`.github/scripts/ci-fake-upstream.sh` 假上游或调用方自有真实上游）启动服务后执行 `make ui-shots`
- THEN `UI_SHOTS_OUT` 下恰有 6 格 × 5 态 × 2 源 = 60 张 PNG 与一个 `index.html`，退出码 0；任何一张缺失、任一格横向溢出或页面 console error 时非零
