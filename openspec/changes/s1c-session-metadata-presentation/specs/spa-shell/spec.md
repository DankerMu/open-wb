...
#### Scenario: 顶栏 actions 插槽
- **WHEN** 一个测试页面经 `useTopbar({ breadcrumb, actions })` 上报标题 `T` 与一个 `label` 为 `示例操作`、`expanded` 为 `false` 的 actions 描述符并位于 `/?session=<id>`；另一测试页面在 `/files` 经 `useTopbar({ actions })` 上报同一描述符但不含 `expanded`；随后移除 `?session=`；再以不提供 `actions` 的上报重复第二态
- **THEN** 第二态与第三态的 banner 内由 shell 渲染的 `示例操作` 按钮（Tooltip `示例操作`）位于 level-1 heading 之后，第二态带 `aria-expanded="false"`、第三态无 `aria-expanded`，点击它恰调用一次 `onSelect` 且参数为该按钮元素；heading 的 accessible name 分别恰为 `我的工作 / T` 与 `工作空间`（不含 `示例操作`）；移除 `?session=` 后 banner（`≤760px` 窄条）与主区都不含该按钮；不提供 `actions` 时 banner 只含既有的 `打开导航`（窄屏）与 heading，既有「顶栏三态」断言保持成立
