...
- **既有夹具迁移（同刀例外）**：该 PR 允许给既有夹具补新键，但只经共享 null 常量（server 侧 `NULL_SESSION_META`、`thinking: null`、`changes: null`，web 侧同名常量放在测试 support 文件），逐文件列于 tasks 5.1；`server/test/session-rest.test.ts`（master 769 行，A 5.3 还会增长）中的五键 `toEqual` 断言**移到**新文件 `session-snapshot-metadata.test.ts` 并改为八键（该文件 −7 行、补键约 +15 行，净约 +8），size-guard ≤ 800 逐刀记录；此后 `session-rest.test.ts` 不再改动。
