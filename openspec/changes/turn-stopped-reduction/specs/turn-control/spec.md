...
> - 「stopped 终态」：三处 CHECK 句已由 1.1 #449（迁移 034）交付，此处原文重述以使该 requirement 完整；「与 web 联合类型」子句归 7.1 #472；「启动对账仍只把 `running` 置为 `failed`，`stopped` 行不受影响」句与 Scenario「停止后继续对话」「对账不触碰 stopped」归 4.2a #473。
