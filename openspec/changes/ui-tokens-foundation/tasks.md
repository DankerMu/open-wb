...
- [x] 2.3 `web/test/ui-guardrails.test.ts`：grep 守卫（features/routes css+tsx 无 `#hex`/`rgba?(`/`--wb-palette-`；`web/src/ui/**/*.tsx` 无 `style={`）+ `motion.css` reduced-motion 文本断言 + `ATTRIBUTION.md` 含 lucide/ISC 与 Radix/MIT；先红（chat.css:5 注释 hex、motion.css 缺失、ATTRIBUTION 未加）。
