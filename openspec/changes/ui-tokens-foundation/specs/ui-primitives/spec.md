# Spec: ui-primitives（切片 #275：token 全集与基元出口）

> 本切片交付 S1e `ui-primitives` capability 的「设计 token 全集」（完整）与「基元组件库」「动效与图标」的首刀子集，Requirement 标题与父 change 一致；#276–#279 以 MODIFIED 扩展「基元组件库」，#295 以 MODIFIED 给「动效与图标」补 ui-walk 断言。
