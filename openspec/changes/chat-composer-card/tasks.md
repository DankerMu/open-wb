...
- [ ] 2.1 新 `web/test/chat-composer.test.tsx` 写 C1–C5 与 C6 新增条；按改写规则 2 全仓 grep 并改写英文会话状态断言；ui-walk 两处期望改中文。先红。
- [ ] 2.2 `status-label.ts`、`composer.tsx`；`conversation-view.tsx` 接入 Composer 与会话项状态元素；`page.tsx` 删常量；`chat.css` 按 design。2.1 转绿，`web/test` 全绿。
- [ ] 2.3 反向注入十三项各红并回退（design Required evidence）。
