# Spec delta: omp-test-harness（#461 fake-omp `slow-ready`）

> 本 delta 并入父 delta 的两条 requirement，并自写一条模块划分 requirement。
> - 「假 omp 进程契约」：以当前主 spec 为底，已含 abort-ok/abort-ignored/branch/approval 三场景与它们的全部 Scenario，全部原样保留。只并入父 delta 的 `slow-ready`/`--ready-delay-ms` 段与 Scenario「延迟握手」，均逐字；场景枚举句追加 `slow-ready`。留给 6.6 #470 归档时并入的有：`approval-chain-abort-ignored` 段、场景枚举的最终形态（父 delta 的「eight scripted scenarios (…)」）、Scenario「审批链上 abort 被忽略」。
> - 「假 omp 入站帧记录」：主 spec 原文，加上 #459 延后的 Scenario「延迟握手期间停止 → 帧序 prompt,abort」（逐字）。加入后与父 delta 逐字一致，没有剩余部分。
> - 「假 omp 夹具模块划分」：父 delta 没有这条，是本 issue 纯拆分的长期约束（先例 session-store-split），归档时原样推进。
...
#### Scenario: 延迟握手期间停止 → 帧序 prompt,abort
- **WHEN** a `slow-ready` child completes its delayed handshake, receives a prompt immediately followed by `abort` (as when its owner recorded a stop intent during the handshake and aborted right after the dispatch receipt), and after that aborted turn receives a probe prompt
- **THEN** the probe `frames=` value is exactly `negotiate_protocol,get_state,prompt,abort,prompt` — the last `prompt` being the probe — so a host test can prove the stopped turn's `prompt` was dispatched and its `abort` followed it

## ADDED Requirements

### Requirement: 假 omp 夹具模块划分
`server/test/support/fake-omp.mjs` SHALL 是假 omp 唯一的可执行入口，负责 argv 解析、JSONL 帧收发、scenario 状态与出站帧。call-proxy 的上游客户端 SHALL 位于同目录的 `server/test/support/fake-omp-proxy.mjs`，职责包括：读取受管 `models.yml` 的 `providers.workbuddy` 配置、向 `baseUrl/chat/completions` 发 POST、重组 SSE 的 delta 与 tool call 分片、解析上游 tool call 参数。`fake-omp-proxy.mjs` SHALL 只导入 `node:` 内建模块，不导入 `fake-omp.mjs`，不持有模块级可变状态，也不发出任何 JSONL 帧；值导入只沿 `fake-omp.mjs → fake-omp-proxy.mjs` 一个方向（有向无环）。两个文件 SHALL 各自 ≤800 行（AGENTS.md 的文件行数约定；`scripts/size-guard.sh` 不扫描 `.mjs`，以 `wc -l` 核对）。夹具整体仍不依赖任何第三方包。

#### Scenario: 模块划分可持续验证
- **WHEN** 检查 `server/test/support/` 下两个模块的 import 语句，并运行 `wc -l` 与 server 测试
- **THEN** `fake-omp-proxy.mjs` 的 import 只含 `node:` 说明符、不含 `fake-omp.mjs`；`fake-omp.mjs` 以相对路径 `./fake-omp-proxy.mjs` 静态导入它；两个文件均 ≤800 行；fake-omp 与 call-proxy 相关测试全绿
