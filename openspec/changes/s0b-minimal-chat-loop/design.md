...
5. **事件契约与回放**（grill ④⑩）：
   - 事件类型：`turn.start{messageId}`、`text.delta{messageId,delta}`、`step.start{messageId,stepId,name,detail}`、`step.end{messageId,stepId,status:"done"|"failed"}`、`turn.end{messageId,status:"done"|"failed"}`、`error{messageId?,message}`、`replay.gap{}`。数据SSE帧为 `id:<streamEpoch>:<seq>`、`event:<type>`、`data:<json>`；gap控制帧为空 `id:` 重置EventSource游标，不占序号；每15s一条 `: keepalive` 注释。
