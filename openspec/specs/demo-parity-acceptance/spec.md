...
- THEN `UI_SHOTS_OUT` 下恰有 6 格 × 5 态 × 2 源 = 60 张 PNG 与一个 `index.html`，退出码 0；任何一张缺失、任一格横向溢出、非 chat 态（`login-default`、`files-readme`、`settings-default`）截图前 DOM 含 workspace `root` 值（chat 态按 ADR-0011 不做该断言）、`settings-default` 截图前文字色或残留过渡断言不成立，或页面 console error 时非零，且已产出文件保留
