...
### What You're Evolving
The agent loop itself: a multi-step workflow, an orchestrator plus sub-agents, a pipeline of prompts, or agent framework code that decides how work is routed and what information moves between steps.

A `harness` differs from a `prompt` or `skill` in that the thing being optimized is the *structure of the loop*, not the text any single step contains. If your artifact has exactly one model call, it is a prompt. If it has several and the interesting question is what passes between them, it is a harness.

### Decomposition
Following RHI (arXiv 2607.15524), decompose into four components before proposing any mutation:

| Component | Definition |
|---|---|
| **Contract** | What information a step passes to the next step or back to the orchestrator |
| **Hop** | The interaction structure — which step runs when, and the workflow's control flow |
| **Role** | Which agent handles which part of the task |
| **Instruction** | What each agent is told to do |

Contracts and hops together are the *workflow*; roles and instructions are the *agent design*. Layer 1 mutates contracts, Layer 2 mutates hops, Layer 3 mutates roles and instructions. See `mutation.md` for why this ordering is inverted relative to other artifact types, and for the caveat that it is a hypothesis.

### Execution Method
`custom` or `shell` — run the full loop end to end on the case prompt and capture the final output plus the intermediate trace. Capturing intermediates matters more here than for other types: a harness failure is usually a step receiving the wrong information, which is invisible in the final output alone.

### Recommended Mode
Pairwise Mode when the harness produces open-ended deliverables and no correctness signal exists. GT Suite when tasks have checkable outcomes. Prefer GT Suite whenever a real signal is available — see the warning in `evaluation.md` about self-judged revision degrading strong models.

### GT Design Tips
- Assert on intermediate state where you can, not just the final answer. A `file_exists` or `json_valid` on a step's handoff catches contract breakage that a final-output assertion misses.
- Include at least one case where a step must *not* receive some piece of information, to catch contracts that leak the whole history back in.
- Cost is a first-class outcome here, not just a guardrail. Record tokens and cache hit rate per case; contract tightening should show up as a cost drop.

### Common Failure Modes
- **Context bloat:** Every iteration adds instructions and nothing removes them, so the loop gets slower and eventually worse. This is the dominant failure mode. Run scheduled deletion mutations.
- **Contract leakage:** A step is handed the full interaction history "just in case." Costs scale with loop length and later steps get distracted by irrelevant earlier detail.
- **Memorized fixes:** Edits that encode the answer to one specific task rather than a reusable rule. Detected by the holdout gate, not by dev.
- **Weak-model mismatch:** See the model-tier prerequisite in SKILL.md. On models below the mid tier, a richer harness can perform worse than a minimal one.
