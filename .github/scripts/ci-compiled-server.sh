...
pid=""; hpid=""; upid=""; primary_rc=0; cleanup_rc=0; wrapper_term=0; killed=0; wait_rc=0; pending=0
sanitize() { sed -E -e 's/[0-9a-fA-F]{64}/[redacted]/g' -e 's/"password":"[^"]*"/"password":"[redacted]"/g'; }
dump_logs() { echo "----- server log (failure) -----" >&2; [ -f "$log" ] && sanitize < "$log" | tail -n 80 >&2; [ -f "${state}/upstream.log" ] && sanitize < "${state}/upstream.log" | tail -n 40 >&2; }
...
on_exit() {
  ...
  if [ -z "$pid" ] && [ -z "$hpid" ] && [ -z "$upid" ]; then trap 'exit 143' TERM INT; fi; [ "$pending" -eq 1 ] && primary_rc=143
...
set -m
bash .github/scripts/ci-fake-upstream.sh >"${state}/upstream.log" 2>&1 &
upid=$!; honor_cancel; set +m
if ! galive "$upid"; then echo "upstream PGID contract failed (pid ${upid})" >&2; cleanup_rc=1; fi
if ! alive "$upid"; then dead_up=$upid; wait "$upid" 2>/dev/null || true; upid=""; echo "upstream exited early (pid ${dead_up})" >&2; dump_logs; primary_rc=1; exit 1; fi
ready_up=0; attempts=0
while [ "$attempts" -lt "$ready_attempts" ]; do
  honor_cancel; alive "$upid" || { echo "upstream exited before readiness (pid ${upid})" >&2; break; }
  if grep -F "\"port\":${FAKE_UPSTREAM_PORT}" "${state}/upstream.log" >/dev/null 2>&1; then
    code=$(curl -sS -o /dev/null -w "%{http_code}" --connect-timeout 1 --max-time 2 -X POST "http://127.0.0.1:${FAKE_UPSTREAM_PORT}/v1/chat/completions" -H "content-type: application/json" -d '{"messages":[]}' || true)
    [ "$code" = "401" ] && { ready_up=1; break; }
  fi
  attempts=$((attempts + 1)); sleep "$ready_sleep"; done
honor_cancel
if [ "$ready_up" -ne 1 ]; then
  if alive "$upid"; then echo "readiness failed against fake upstream (pid ${upid})" >&2
...
while [ "$attempts" -lt "$ready_attempts" ]; do
  honor_cancel
  alive "$upid" || { echo "upstream exited before app readiness (pid ${upid})" >&2; break; }
...
if [ "$pending" -eq 0 ] && [ -n "$upid" ] && ! alive "$upid"; then echo "upstream exited early (pid ${upid})" >&2; [ "$primary_rc" -eq 0 ] && primary_rc=1; fi
set -e
