...
> 2026-09-24 S1e grill 已全部拍板，结论记录于 `IMPLEMENTATION_PLAN.md`（S1c Outcome 与前端通用契约）与 `openspec/changes/s1e-frontend-parity/`：1 → 显示逻辑路径 `<账号>/<dir>`；2 → 自有字标 + 现有 mark 占位，资产另议；3 → 重新生成/审批条/对话内搜索归 S1c，麦克风/⌘K/赞踩明确不做；4 → 仅 `dev-stub` 时渲染。以下为拍板前原文。
